...
A dedicated Python package decoupled from Django models.
**Flow:**
1.  **Input**: Job Description text + List of Resume texts.
2.  **Preprocessing**: Lowercase -> Remove URLs/Special Chars -> Tokenize -> Remove Stopwords -> Lemmatize.
3.  **Vectorization**: `TfidfVectorizer` converts text to numerical vectors.
4.  **Ranking**: `cosine_similarity` calculates the angle between the Job Vector and each Resume Vector.
5.  **Output**: List of scores (0.0 to 1.0).
...
sequenceDiagram
    ...
    User->>API: POST /api/resumes/ (File)
    API->>API: Extract Text (PDF/DOCX)
    API->>DB: Save Resume + Parsed Text
    DB-->>API: Resume ID
    
    User->>API: POST /api/screening/screen/{job_id}/
    API->>DB: Fetch Job & All Resumes
    DB-->>API: Data
    API->>ML: rank_resumes(job_text, resume_texts)
    ML->>ML: Preprocess -> TF-IDF -> Cosine Sim
    ML-->>API: [Scores]
    API->>DB: Upsert ScreeningResult
    API-->>User: JSON List of Ranked Candidates
```

## 4. Authentication Flow (JWT)

1.  **Login**: User posts `username`/`password` to `/api/auth/login/`.
2.  **Token**: Server validates credential and issues `access` (short-lived) and `refresh` (long-lived) tokens.
3.  **Protection**: All sensitive endpoints (Upload, Create Job) require `Authorization: Bearer <access_token>` header.
4.  **Frontend**: Stores token in `localStorage` and attaches it to Fetch requests.
